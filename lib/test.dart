import 'dart:io';

import 'package:ashghal_app_frontend/core/services/app_services.dart';
import 'package:ashghal_app_frontend/app_library/public_request/pagination_request.dart';
import 'package:dio/dio.dart' as dio_p;
import 'package:flutter/material.dart';
import 'package:get/get.dart';
import 'package:image_picker/image_picker.dart';
import 'package:flutter_cache_manager/flutter_cache_manager.dart';

import 'features/post/data/repositories/post_repository_impl.dart';
import 'features/post/domain/Requsets/post_request/add_update_post_request.dart';
import 'features/post/domain/repositories/post_repository.dart';

class Test extends GetView<MainController> {
  const Test({super.key});

  @override
  Widget build(BuildContext context) {
    Get.lazyPut(() => MainController());
    return Scaffold(
      appBar: AppBar(
        backgroundColor: Theme.of(context).colorScheme.inversePrimary,
        title: const Text("Send Files to API"),
      ),
      body: Center(
        child: ListView(
          // mainAxisAlignment: MainAxisAlignment.center,
          children: <Widget>[
            Obx(() => CircleAvatarImage(
                  imagePath: controller.imagePath.value,
                  radius: 100,
                  onPressedIcon: controller.getImage,
                )),
            ElevatedButton(
                onPressed: () => controller.uploadImage(),
                child: const Text("Upload Image")),
            ElevatedButton(
                onPressed: () => controller.getData(),
                child: const Text("get Image")),
            const Divider(thickness: 2),
            ElevatedButton(
                onPressed: () async {
                  try {

                    // var paths= controller.images.map((image) => image.path).toList();
                    // print("<<<<<<<<<<<<<<<<$paths>>>>>>>>>>>>>>>>");
                    // AppServices p= Get.find();
                    // p.prefs.clear();
                    PostRepository ds = PostRepositoryImpl();
                    var result = await  ds.getAllCompletePosts(PaginationRequest(pageNumber: 1, perPage: 1));
                    // var result = (await ds.addPost(
                    //   AddPostRequest(
                    //     title: "title",
                    //     content: "Upload more one multimedia",
                    //     categoryId: 1,
                    //     multimediaPaths: paths
                    //   ),
                    // ));
                    result.fold((l) {
                      print(l.message);
                    }, (r) {
                      print(":::::mult: ${r}");
                    },);
                    
                  } catch (e) {
                    print(">>>>>>>>>>>error<<<<<<<<<<<:$e");
                  }
                },
                child:
                    const Text("Try", style: TextStyle(color: Colors.white))),
            Obx(
              () => controller.data.isNotEmpty
                  ? ShowBox(controller.data)
                  : const Center(
                      child: Text("Not found response"),
                    ),
            ),
          ],
        ),
      ), // This trailing comma makes auto-formatting nicer for build methods.
    );
  }

  Widget ShowBox(Map<String, Object?> data) {
    return Card(
      child: Column(children: [
        Text("Name arabic: ${data['name_ar']}"),
        Text("Name english: ${data['name_en']}"),
        Text("Descreption arabic: ${data['desc_ar']}"),
        Text("Descreption english: ${data['desc_en']}"),

        // CircleAvatarImage(
        //     imagePath: data['image'].toString(), onPressedIcon: () {}),
        FutureBuilder<Widget>(
          future: controller.loadImage(
              "data:image/jpeg;base64,"),
          builder: (context, snapshot) {
            try {
              if (snapshot.connectionState == ConnectionState.waiting) {
                print(">>>>>>>>>>>>>>loading");
                return CircularProgressIndicator();
              } else if (snapshot.hasData && snapshot.data == null) {
                print(">>>>>>>>>>>>>>>>>>>>>>>>null snapshot");
                return const Center(child: Text("Not found image"));
              } else {
                print(">>>>>>>>>>>>>>>>>>>>>>>>>>hhhhh");
                return snapshot.data!;
              }
            } catch (e) {
              print("<><><><><><><><><><><><><><><><><><><><><><><><>>");
            }
            return Text("Form feature builder");
          },
        )
      ]),
    );
  }
}

Widget CircleAvatarImage(
    {required String imagePath,
    double radius = 100,
    required Function onPressedIcon}) {
  return Container(
    width: 100,
    child: Stack(
      alignment: Alignment.center,
      children: [
        Container(
          decoration: BoxDecoration(
            shape: BoxShape.circle,
            border: Border.all(color: Colors.black87, width: 5),
          ),
          child: CircleAvatar(
            radius: radius,
            child: imagePath.isEmpty
                ? null
                : ClipRRect(
                    borderRadius: BorderRadius.circular(100),
                    child: Container(
                      decoration: BoxDecoration(shape: BoxShape.circle),
                      child: Image.file(
                        File(imagePath),
                        fit: BoxFit.fitWidth,
                      ),
                    ),
                  ),
          ),
        ),
        Positioned(
          bottom: 20,
          right: 20,
          child: CircleAvatar(
            radius: 30,
            backgroundColor: Colors.black,
            child: IconButton(
              icon: const Icon(Icons.photo_camera_back_rounded,
                  size: 40, color: Colors.white),
              onPressed: () => onPressedIcon(),
            ),
          ),
        )
      ],
    ),
  );
}

class MainController extends GetxController {
  RxString imagePath = "".obs;
  RxMap<String, Object?> data = RxMap();
  List<XFile> images = [];

  /// get image from image picker
  Future<void> getImage() async {
    final ImagePicker _picker = ImagePicker();
    // Pick an image
    images = await _picker.pickMultiImage();
    //TO convert Xfile into file
    if (images != null) {
      print("Image picked");
      imagePath.value = images[0].path;
    }
  }

  /// upload request contains image to server
  uploadImage() async {
    if (imagePath.isEmpty) return;
    dio_p.Dio dio = dio_p.Dio();
    dio.interceptors
        .add(dio_p.LogInterceptor(requestBody: true, responseBody: true));
    try {
      print("?????????????????: ${imagePath.value}");
      var imagesData = dio_p.FormData();
      for (var file in images) {
        imagesData.files.add(
          MapEntry("multimedia", await dio_p.MultipartFile.fromFile(file.path)),
        );
      }
      print(">>>>>>>>${[imagesData.files]}");
      print(imagePath.value);
      var formData = dio_p.FormData.fromMap({
        'title': 'page1',
        'content': 'page1',
        'category_id': 1,
        'desc_en': "the first page",
        'multimedia': [
          await dio_p.MultipartFile.fromFile(imagePath.value),
          await dio_p.MultipartFile.fromFile(images[1].path)
        ],
        // 'multimedia2': dio_p.MultipartFile.fromFile(imagePath.value),
        // await dio_p.MultipartFile.fromFile("E:\\0Flutter_App\\upload_files_to_api\\assets\\icons\\Frame.png"),
      }, dio_p.ListFormat.multiCompatible);
      // var formData = dio_p.FormData.fromMap({
      //   'title': 'page1',
      //   'content': 'page1',
      //   'category_id': 1,
      //   'desc_en': "the first page",
      //   'multimedia': [await dio_p.MultipartFile.fromFile(imagePath.value),await dio_p.MultipartFile.fromFile(images[1].path) ],
      //   // 'multimedia2': dio_p.MultipartFile.fromFile(imagePath.value),
      //   // await dio_p.MultipartFile.fromFile("E:\\0Flutter_App\\upload_files_to_api\\assets\\icons\\Frame.png"),
      // }, dio_p.ListFormat.multiCompatible);
      final response = await dio.post("http://10.0.2.2:8000/api/post/add",
          data: formData,
          options: dio_p.Options(headers: {
            'Accept': 'application/json',
            'x_api_key': 'z5Y4eX4SGe1Be1sLhiHc40Ezw8zdVI',
            'x_language': 'en',
            'Authorization':
                'Bearer 11|lqiuvu32MVrFhHfg60RALlowznZ5tuJHE3mxpdOme2c0bd5e'
          }));
      if (response.statusCode == 200) {
        var map = response.data['data'] as Map;
        print(map);
        print('success');
        if (response.data['status']) {
          data['title_ar'] = map['title_ar'];
          data['title_en'] = map['title_en'];
          data['desc_ar'] = map['desc_ar'];
          data['desc_en'] = map['desc_en'];
          data['image'] =
              map['image_url']?.replaceFirst("localhost", "10.0.2.2");
          print(">>>>>>>>>>>>>>>> The image Path is:: ${data['image']}");
          return true;
        } else {
          return false;
        }
      } else if (response.statusCode == 200) {
        //BotToast is a package for toasts available on pub.dev
        print('>>>>>>>>>>>>>>>>>>>>>>>>Validation Error Occurs');
        return false;
      }
    } on dio_p.DioException catch (error) {
      print(error.message);
      // throw YourException(error);
    } catch (e) {
      // log(_.toString());
      print('Something Went Wrong: $e');
    }
  }

  /// get data from server
  getData() async {
    // if (imagePath.isEmpty) return;
    dio_p.Dio dio = dio_p.Dio();
    dio.interceptors
        .add(dio_p.LogInterceptor(requestBody: true, responseBody: true));
    try {
      final response = await dio.get(
        "http://10.0.2.2:8000/api/onboarding/get/5",
      );
      if (response.statusCode == 200) {
        var map = response.data['data'] as Map;
        print(map);
        print('success');
        if (response.data['status']) {
          data['title_ar'] = map['title_ar'];
          data['title_en'] = map['title_en'];
          data['desc_ar'] = map['desc_ar'];
          data['desc_en'] = map['desc_en'];
          data['image'] =
              map['image_url']?.replaceFirst("localhost", "10.0.2.2");
          print(">>>>>>>>>>>>>>>> The image Path is:: ${data['image']}");
          return true;
        } else {
          return false;
        }
      } else if (response.statusCode == 200) {
        //BotToast is a package for toasts available on pub.dev
        print('>>>>>>>>>>>>>>>>>>>>>>>>Validation Error Occurs');
        return false;
      }
    } on dio_p.DioException catch (error) {
      print(error.message);
      // throw YourException(error);
    } catch (_) {
      // log(_.toString());
      print('Something Went Wrong');
    }
  }

  /// get image from cashe or download it if not found in cashe and cashed it
  Future<String?> getDownloadCachedPath(String imageUrl) async {
    try {
      print("??????????????????????????????????????????");
      DefaultCacheManager cacheManager = DefaultCacheManager();
      print("><<<<<<<<<<<<<<<<<<<<<<<<<<<<<..........");
      FileInfo? fileInfo = await cacheManager.getFileFromCache(imageUrl);
      // File fileInfo = await DefaultCacheManager().getSingleFile(imageUrl);
      print("HHHHHHHHHHHHHHHHHHHHHHH:${fileInfo}");
      print("....................................");
      if (fileInfo == null) {
        // Image is not cached, download and store it locally.
        await cacheManager.downloadFile(imageUrl);
        fileInfo = await cacheManager.getFileFromCache(imageUrl);
      }
      return fileInfo!.file.path; // Return the local path of the cached image.
    } catch (e) {
      print("::::::::: Error in cashedPath: $e");
    }
    return null;
  }

  /// build image widget
  Future<Widget> loadImage(String imageUrl) async {
    try {
      String? imagePath = await getDownloadCachedPath(imageUrl);
      print("<>>>>>>>>>>>????????$imagePath>");
      if (imagePath != null) {
        print("<>>>>>>>>>>>$imagePath>");
        return Image.file(File(imagePath));
      }
    } catch (e) {
      print("::::::::: Error: $e");
      // AppUtil.showMessage(AppLocalization.thereIsSomethingError, Colors.green);
    }
    return const Center(child: Text("Not fount Image"));
  }
}
