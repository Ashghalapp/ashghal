...
class SpecificUserAccountController extends GetxController {
  ...
  // Map<String, dynamic> get getCurrentUserDataOffline =>
  //     SharedPref.getCurrentUserData();
  ...
  Future<void> getSpecificUserData(int userId) async {
    ...
    (await getSpecificUserUS.call(userId)).fold((failure) {
      ...
      // isRequestFinishWithoutData = true;
      userData.value = user;
      print(">>>>>>>>>>>>>>>>>Done get specific $userId User Data>>>>>>>>>>>>>>");
    });
  }
  ...
  PopupMenuButtonWidget getPostMenuButtonValuesWidget(int postId) {
    return PopupMenuButtonWidget(
      values: OperationsOnCurrentUserPostPopupMenuValues.values
          .asNameMap()
          .keys
          .toList(),
      onSelected: (value) {
