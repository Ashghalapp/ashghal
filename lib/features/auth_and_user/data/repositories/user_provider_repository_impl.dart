...
import 'package:ashghal_app_frontend/features/auth_and_user/data/data_sources/remote_data_source.dart';
...
class UserProviderRepositoryImpl extends UserProviderRepository {
  ...
  Future<Either<Failure, Success>> sendEmailVerificationCode(String email) async {
    var result = await _handleErrors(() async {
      return remoteDataSource.sendEmailVerificationCode(email);
    });
    return result is Success? Right(result): Left(result);
  }

  @override
  Future<Either<Failure, Success>> validateEmailVerificationCode(ValidateEmailCodeRequest request) async {
  ...
  Future<Either<Failure, Success>> addOrChangeEmail(AddOrChangeEmailRequest request) async{
    var result= await _handleErrors(() async{
      return await remoteDataSource.addOrChangeEmail(request);
    });
    return result is Success? Right(result): Left(result);
  }
