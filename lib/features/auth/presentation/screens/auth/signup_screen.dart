...
import 'package:ashghal/features/auth/presntaion/getx/Auth/singup_controller.dart';
import 'package:ashghal/features/auth/presntaion/widgets/social_icons.dart';
            ...
            Form(
              ...
              child: Column(
                children: [
                  ...
                  controller.hanldlingWaitingRequest(
                    isLoading: isLoading,
                    child: MyGesterDedector(
                      text: '17'.tr,
                      ...
                      onTap: () async {
                        try {
                          ...
                          var isValid = controller.signUpFormKey.currentState!.validate();
                          if (isValid) {
                            if (!await controller.isEmailExist()) {
                              if (isProviderSignUp ?? false) {
                                Get.toNamed(AppRoutes.singUpScreenJob);
                              } else if (await controller.registerUser()){                                
                                Get.toNamed(AppRoutes.verficationSignUp);
                              }
                            } else{
                              // showSnackBar("Error", "The email has already been taken");
                            }
                          }
                        } catch (e) {
            ...
            children: [
                ...
                child: Column(
                  children: [
                    DropdownButtonFormField(
                      ...
                      validator: (value) => validInput(value?.toString()??"", null, null, 'selectedCategory'),
                      onChanged: (value) {
                        controller.jobCategoryController.text = value?.toString() ?? "";
                      },
                    ),
                    const SizedBox(height: 20),
              ...
              controller.hanldlingWaitingRequest(
                ...
                child:MyGesterDedector(
                  ...
                  onTap: () async {                    
                    isLoading.value = true;                  
                    bool isValid = controller.jobFormKey.currentState?.validate() ?? false;
                    if (isValid) {
                      if (await controller.registerProvider()) {
                        Get.toNamed(AppRoutes.verficationSignUp);
                        // Get.toNamed(AppRoutes.succesSignUp);
                        // Get.offAllNamed(AppRoutes.logIn);
                      }
                    }
                    isLoading.value= false;
                    // Get.toNamed(AppRoutes.singUpScreenEmail);
                  },
                ),
              ),
              SizedBox(
